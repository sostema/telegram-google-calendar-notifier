...
def main():
    ...
    logger = src.utils.setup_logger("telegram")
    ...
    stop_notify_handler = CommandHandler(
        "stop_notify", stop_notify, pass_job_queue=True
    )
    ...
    job_queue.run_repeating(
        src.tele.helpers.save_jobs_job, datetime.timedelta(minutes=1)
    )

    try:
        src.tele.helpers.load_jobs(job_queue)
    ...
    src.tele.helpers.save_jobs(job_queue)


def send_timetable(context, date, chat_id):
    calendar = src.CalendarAPI.helpers.setup_calendar_from_config(config)
...
def date_timetable(update, context):
    if context.args:
        ...
        try:
            date = src.time_helpers.Date().parse_datetime(date_str)
        except Exception as e:
        ...
        date = src.time_helpers.Date(full_day=True)
        send_timetable(context, date, update.message.chat_id)


def now_timetable(update, context):
    date = src.time_helpers.Date(datetime.datetime.now(), False)
    send_timetable(context, date, update.message.chat_id)


def today_timetable(update, context):
    date = src.time_helpers.Date(datetime.datetime.today(), True)
    send_timetable(context, date, update.message.chat_id)


def tomorrow_timetable(update, context):
    date = src.time_helpers.Date(
        datetime.datetime.today() + datetime.timedelta(1), True
    )
    send_timetable(context, date, update.message.chat_id)
...
def notify(update, context):
    if context.args:
        try:
            delay = datetime.datetime.strptime(context.args[0], "%H:%M").replace(
                year=datetime.datetime.today().year,
                month=datetime.datetime.today().month,
                day=datetime.datetime.today().day,
            )
            delay = src.time_helpers.local_timezone.localize(delay)
            logger.debug(delay)
        except Exception as e:
            pass
            context.bot.send_message(
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                "Сегодня", callback_data=f"{delay.strftime('%H:%M')}_1"
            ),
            InlineKeyboardButton(
                "Завтра", callback_data=f"{delay.strftime('%H:%M')}_2"
            ),
        ],
        [
            InlineKeyboardButton(
                "Сегодня и завтра", callback_data=f"{delay.strftime('%H:%M')}_3"
            )
        ],
    ]
    ...
    context.bot.send_message(
        chat_id=update.message.chat_id,
        text=f"Выберите вид расписания: ",
...
def notify_button(update, context):
    ...
    date = datetime.datetime.strptime(data[0], "%H:%M").replace(
        year=datetime.datetime.today().year,
        month=datetime.datetime.today().month,
        day=datetime.datetime.today().day,
    )
    date = (pytz.utc.localize(date)).astimezone(src.time_helpers.local_timezone)
